...
public class RabbitMqConfig {
    ...
    public Declarables queueDeclarables() {
        ...
        TopicExchange userExchange = new TopicExchange(MESSAGE_EXCHANGE);
        Queue firstQueue = new Queue(MESSAGE_QUEUE);
        declarables.add(firstQueue);
        declarables.add(userExchange);

        declarables.addAll(getDeclarables(MESSAGE_QUEUE, userExchange));
        ...
        Queue userQueue = declarables.stream().filter(declarable -> {
            ...
            if (declarable instanceof Queue) {
                ...
                response = queue.getName().equals(MESSAGE_QUEUE);
            }
            return response;
